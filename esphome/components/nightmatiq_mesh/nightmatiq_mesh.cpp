...
void NightmatiqMesh::begin_identity_scan_() {
  ...
  if (state == esp32_ble_tracker::ScannerState::RUNNING) {
    ...
    // lets it select the random scanner address required by NightmatIQ.
    tracker->stop_scan();
  }
}
...
void NightmatiqMesh::advance_identity_scan_() {
  ...
  if (tracker == nullptr) {
    ...
    // Restore the project's normal Bluetooth Proxy scan parameters before
...
bool NightmatiqMesh::initialize_mesh_() {
  ...
  this->set_status_("Enabling Bluetooth Mesh bearer");
  error = esp_ble_mesh_provisioner_prov_enable(ESP_BLE_MESH_PROV_ADV);
  if (error != ESP_OK) {
    ESP_LOGE(TAG, "Mesh advertising bearer enable failed: %s", esp_err_to_name(error));
    this->set_status_(std::string("Mesh advertising bearer enable failed: ") + esp_err_to_name(error));
    return false;
  }
  ...
  this->set_status_("Waiting for Bluetooth Mesh provisioner");
  return true;
}
...
void NightmatiqMesh::setup() {
  ...
  if (!this->mesh_mode_enabled_) {
    this->set_status_("NightmatIQ disabled; Bluetooth Proxy active");
    return;
  }
  this->actual_output_forced_unavailable_.store(false);
  if (bluetooth_proxy::global_bluetooth_proxy != nullptr)
    bluetooth_proxy::global_bluetooth_proxy->set_active(false);
...
void NightmatiqMesh::dump_config() {
  ...
  ESP_LOGCONFIG(TAG, "  BLE runtime mode: %s", this->mesh_mode_enabled_ ? "Bluetooth Mesh" : "Bluetooth Proxy");
...
void NightmatiqMesh::bind_model_(uint16_t model_id) {
  const esp_err_t error = esp_ble_mesh_provisioner_bind_app_key_to_local_model(
      this->config_.local_address, this->config_.app_key_index, model_id, ESP_BLE_MESH_CID_NVAL);
  if (error != ESP_OK)
...
void NightmatiqMesh::keys_bound_() {
  ...
  // that fact across AR01V3 restarts and start normal polling promptly.
